...
export const generateGasBillPDF = async (billData: GasBillData) => {
  const doc = new jsPDF()
  ...
  doc.text(`${businessName.toUpperCase()} - GAS BILL`, pageWidth / 2, yPos, {
    align: 'center',
  })
  ...
  unifiedBody.push([
    { content: 'TotalGas', styles: { fontStyle: 'bold' } },
  ...
  yPos += 5
  doc.setFontSize(8)
  doc.text('This gas bill includes the following charges:', margin, yPos)
  yPos += 4
  ...
  const charges = [
    ...
    '• Government Taxes',
    '• Surcharge',
    '• Service & Infrastructure charges',
  ]
  ...
  // ==================== DOWNLOAD ====================
  const fileName = `Gas_Bill_${billData.reading.bill_number.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`
  doc.save(fileName)
}
