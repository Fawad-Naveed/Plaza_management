...
export interface MaintenanceBillData {
  reading: {
    id: string
    business_id: string
    bill_number: string
...
export const generateMaintenanceBillPDF = async (billData: MaintenanceBillData) => {
  const doc = new jsPDF()
  ...
  doc.text(`${businessName.toUpperCase()} - MAINTENANCE BILL`, pageWidth / 2, yPos, {
    align: 'center',
  })
  ...
  // Maintenance Information + Payment History headers
  unifiedBody.push([
    { content: 'Month', colSpan: 1, styles: { fontStyle: 'bold', halign: 'center' } },
    { content: 'Maintenance', colSpan: 1, styles: { fontStyle: 'bold', halign: 'center' } },
  ...
  // Current maintenance + history rows
  const maxRows = Math.max(1, historyRows.length)
  for (let i = 0; i < maxRows; i++) {
    const row: any[] = []
    
    if (i === 0) {
      // First row: current maintenance data
      row.push(
        monthYear,
        billData.reading.maintenance_charge.toFixed(0),
  ...
  unifiedBody.push([
    { content: 'Maintenance Charge', styles: { fontStyle: 'bold' } },
    { content: billData.reading.maintenance_charge.toFixed(0), colSpan: 2 },
    { content: 'CURRENT BILL', styles: { fontStyle: 'bold' } },
    { content: billData.reading.amount.toFixed(0), colSpan: 2, styles: { halign: 'right' } },
  ])
  
  unifiedBody.push([
    { content: 'Total Maintenance', styles: { fontStyle: 'bold' } },
  ...
  doc.autoTable({
    ...
    head: [
      [
        { content: 'Maintenance Information', colSpan: 3, styles: { fontStyle: 'bold', halign: 'center', fillColor: [0, 150, 136], textColor: [255, 255, 255] } },
  ...
  yPos += 5
  doc.setFontSize(8)
  doc.text('This maintenance bill includes the following charges:', margin, yPos)
  yPos += 4
  ...
  // ==================== DOWNLOAD ====================
  const fileName = `Maintenance_Bill_${billData.reading.bill_number.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`
  doc.save(fileName)
}
